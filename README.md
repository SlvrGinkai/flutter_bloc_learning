...
handler that was registered, and _that_ handler decides the new
...
| Traceability                            | Only via `onChange` (state diffs)                     | Via `onEvent` _and_ `onChange` (cause + effect)                                           |
...
- **Reached for Cubit by default.** Most local UI state (a counter, a
  ...
  doesn't need a named-event vocabulary — the method call _is_ the
